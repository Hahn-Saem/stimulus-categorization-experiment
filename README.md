...
Other than the main function, I plan to have a function that will run the experiment. The function will display the target as a word first, then show the image and distractor images, following the details in the second paragraph of the project description.

I also plan to include a function that gets the next set of category items and returns them. The next set of items will then be passed as an argument to the function that runs the experiment.

A third function I hope to include is a function that will record the times for each image pair. That is, the time it takes for the participant to click on a key when the target and the distractor image are displayed on the screen. I might also keep track of whether the participant chose the correct item in this function.
